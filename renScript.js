...
function setLayout() {
    ...
    if(hand=='left') { // LH tools and dialogs
        id('swop').setAttribute('href','renStyleLeft.css');
        id('prompt').style.top='48px';
        id('goofy').checked=false;
    }
    else { // RH tools and dialogs
        id('swop').setAttribute('href','renStyleRight.css');
...
// START-UP CODE
var request=window.indexedDB.open("renDB",dbVersion);
request.onsuccess=function(event) {
...
if (navigator.serviceWorker.controller) {
	...
	navigator.serviceWorker.register('renSW.js').then(function(reg) {
