...
function setLayout() {
    ...
    if(hand=='left') { // LH tools and dialogs
        id('swop').setAttribute('href','wrenStyleLeft.css');
        id('prompt').style.top='48px';
        id('goofy').checked=false;
    }
    else { // RH tools and dialogs
        id('swop').setAttribute('href','wrenStyleRight.css');
...
// START-UP CODE
var request=window.indexedDB.open("wrenDB",dbVersion);
request.onsuccess=function(event) {
...
if (navigator.serviceWorker.controller) {
	...
	navigator.serviceWorker.register('wrenSW.js').then(function(reg) {
